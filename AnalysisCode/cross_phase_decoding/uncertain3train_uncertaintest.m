...
ds = basic_DS('uncertain3-gonogo','stimulus_ID', 3);ds1 = basic_DS('uncertain1-gonogo','stimulus_ID', 3);ds2 = basic_DS('uncertain2-gonogo','stimulus_ID', 3);
ds.randomly_shuffle_labels_before_running=0;
cl =libsvm_CL;
fps{1} = zscore_normalize_FP;
cv = standard_resample_CV(ds, cl, fps);cv1 = standard_resample_CV(ds1, cl, fps);cv2 = standard_resample_CV(ds2, cl, fps);
cv.num_resample_runs = 500;
cv.display_progress.zero_one_loss = 0;
cv.test_only_at_training_times = 1;
datasource = cv.datasource;datasource1 = cv1.datasource;datasource2 = cv2.datasource;
    ...
    for iCV = 1:size(all_XTr{1}, 2)
        for iTrainingInterval = 1:size(all_XTr, 2)
            ...
            for iTestInterval = test_interval
                ...
                DECODING_RESULTS.ZERO_ONE_LOSS_RESULTS3.decoding_results(iResample, iCV, iTrainingInterval, iTestIntervalSaveInd, :) = ((length(find(predicted_labels - YTe == 0))/length(predicted_labels)));
                % storing stdevs of single 0-1 classifier results
                DECODING_RESULTS.ZERO_ONE_LOSS_RESULTS3.stdev.all_single_CV_vals(iResample, iCV, iTrainingInterval, iTestIntervalSaveInd) = std(~((predicted_labels - YTe) == 0));   % can also be calculated analytically from decoding_results using p(1-p) binomial variance formula
...
save('uncetrain3_result','DECODING_RESULTS')
