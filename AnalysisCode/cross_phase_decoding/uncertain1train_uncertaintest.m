...
ds = basic_DS('uncertain1-gonogo','stimulus_ID', 3);ds2 = basic_DS('uncertain2-gonogo','stimulus_ID', 3);ds3 = basic_DS('uncertain3-gonogo','stimulus_ID', 3);
ds.randomly_shuffle_labels_before_running=0;
cl =libsvm_CL;
fps{1} = zscore_normalize_FP;
cv = standard_resample_CV(ds, cl, fps);cv2 = standard_resample_CV(ds2, cl, fps);cv3 = standard_resample_CV(ds3, cl, fps);
cv.num_resample_runs = 500;
cv.display_progress.zero_one_loss = 0;
cv.test_only_at_training_times = 1;
datasource = cv.datasource;datasource2 = cv2.datasource;datasource3 = cv3.datasource;
...
for iResample = 1:cv.num_resample_runs
    ...
    [all_XTr2 all_YTr2 all_XTe2 all_YTe2] = datasource2.get_data;
    ...
    for iCV = 1:size(all_XTr{1}, 2)
        for iTrainingInterval = 1:size(all_XTr, 2)
            ...
            for iTestInterval = test_interval
                ...
                DECODING_RESULTS.ZERO_ONE_LOSS_RESULTS1.decoding_results(iResample, iCV, iTrainingInterval, iTestIntervalSaveInd, :) = ((length(find(predicted_labels - YTe == 0))/length(predicted_labels)));
                % storing stdevs of single 0-1 classifier results
                DECODING_RESULTS.ZERO_ONE_LOSS_RESULTS1.stdev.all_single_CV_vals(iResample, iCV, iTrainingInterval, iTestIntervalSaveInd) = std(~((predicted_labels - YTe) == 0));   % can also be calculated analytically from decoding_results using p(1-p) binomial variance formula
                ...
                % test uncertain2 data
                XTe2 = all_XTe2{iTestInterval}{iCV};
                if iscell(all_YTe2)
                    YTe2 = all_YTe2{iTestInterval};
                else
                    YTe2 = all_YTe2;
                end
                % apply feature preprocessing to the test data
                if ~isempty(cv.feature_preprocessors)
                    for iFP = 1:length(cv.feature_preprocessors)
                        XTe2 = feature_preprocessors{iFP}.preprocess_test_data(XTe2);
                    end
                end
                % test the classifier
                [predicted_labels decision_values] = classifier.test(XTe2);
                % store information for creating a confusion matrix
                if cv.confusion_matrix_params.create_confusion_matrix
                    all_predicted_labels_for_confusion_matrix_and_MI(iResample, iCV, :, iTrainingInterval, iTestIntervalSaveInd) = predicted_labels;
                end
                % save the zero-one loss results
                DECODING_RESULTS.ZERO_ONE_LOSS_RESULTS2.decoding_results(iResample, iCV, iTrainingInterval, iTestIntervalSaveInd, :) = ((length(find(predicted_labels - YTe2 == 0))/length(predicted_labels)));
                % storing stdevs of single 0-1 classifier results
                DECODING_RESULTS.ZERO_ONE_LOSS_RESULTS2.stdev.all_single_CV_vals(iResample, iCV, iTrainingInterval, iTestIntervalSaveInd) = std(~((predicted_labels - YTe2) == 0));   % can also be calculated analytically from decoding_results using p(1-p) binomial variance formula
                curr_total_CV_zero_one_loss_results{iTrainingInterval,iTestIntervalSaveInd} = [curr_total_CV_zero_one_loss_results{iTrainingInterval,iTestIntervalSaveInd}; ~((predicted_labels - YTe2) == 0)];
...
save('uncetrain1_result','DECODING_RESULTS')
