...
ds = basic_DS('stable-gonogo','stimulus_ID', 3);ds1 = basic_DS('uncertain1-gonogo','stimulus_ID', 3);
ds2 = basic_DS('uncertain2-gonogo','stimulus_ID', 3);ds3 = basic_DS('uncertain3-gonogo','stimulus_ID', 3);
ds.randomly_shuffle_labels_before_running=0;
cl =libsvm_CL;
fps{1} = zscore_normalize_FP;
cv = standard_resample_CV(ds, cl, fps);cv1 = standard_resample_CV(ds1, cl, fps);
cv2 = standard_resample_CV(ds2, cl, fps);cv3 = standard_resample_CV(ds3, cl, fps);
cv.num_resample_runs = 500;
cv.display_progress.zero_one_loss = 0;
cv.test_only_at_training_times = 1;
datasource = cv.datasource;datasource1 = cv1.datasource;
datasource2 = cv2.datasource;datasource3 = cv3.datasource;
...
DECODING_RESULTS.ZERO_ONE_LOSS_RESULTS.mean_decoding_results = squeeze(mean(mean(DECODING_RESULTS.ZERO_ONE_LOSS_RESULTS.decoding_results*100, 2), 1));
DECODING_RESULTS.ZERO_ONE_LOSS_RESULTS.stdev.over_resamples = squeeze(std(mean(DECODING_RESULTS.ZERO_ONE_LOSS_RESULTS.decoding_results*100, 2), [], 1));
...
save('stable_result','DECODING_RESULTS')
