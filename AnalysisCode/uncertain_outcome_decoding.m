...
for i=1:549
data{i}=[mean(reward.hitDF{i}(16:45,:),'omitnan')';mean(reward.roDF{i}(16:45,:),'omitnan')'];
s=[ones(size(mean(reward.hitDF{i}(16:45,:),'omitnan'),2),1)*1;ones(size(mean(reward.roDF{i}(16:45,:),'omitnan'),2),1)*2];
n=length(s);
stimulus_ID{i}=cell(1,n);
for ii=1:n
    if s(ii,:)==1
        stimulus_ID{i}{ii}='hit';
    else 
        stimulus_ID{i}{ii}='ro';
    end
end
end
save('outcome_data','stimulus_ID','data')
%%
clear
load('uncertain_outcomecell_identify.mat')
...
for i=1:1000
    ...
    save('outcomedecoding1','binned_data','binned_labels','binned_site_info')
    ds = basic_DS('outcomedecoding1','stimulus_ID', 5);
    ...
    save('outcomedecoding2','binned_data','binned_labels','binned_site_info')
    ds = basic_DS('outcomedecoding2','stimulus_ID', 5);
    ds.randomly_shuffle_labels_before_running=0;
    cl =libsvm_CL;
    fps{1} = zscore_normalize_FP;
    cv = standard_resample_CV(ds, cl, fps);
...
save('outcomedecodingresult','result')